...
class AppDelegate: UIResponder, UIApplicationDelegate {
    ...
    let fetchRequest = NSFetchRequest<Person>(entityName: "Person")
        ...
        do{
            arrPerson = try persistentContainer.viewContext.fetch(fetchRequest)
        }catch{
            print(error)
        }
        return arrPerson
    ...
    func deleteRecord( person : Person){
        persistentContainer.viewContext.delete(person)
        saveContext()
    }
    
    func updateRecord(person:Person,name:String,age:Int,tution: Double,startDate: Date){
